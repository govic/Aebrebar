...
async function getFiltros() {
  ...
  let data = await consulta_filtro(filtrado)
  ...
  data = await consulta_filtro(filtrado);
...
function getValFiltro(filtro_name) {
  ...
  consulta_filtro(filtrado).then((data) => {
    ...
    // inserta opciones de filtro  para AEC PARTICIÓN HA
    console.log("VALORES AEC PISO ! PRE CONSULTAS");
  ...
  */
  if (valor_fil2 !== "" && (valor_fil1 === "" || valor_fil1 === 'sinvalor')) { // piso
    ...
    consulta_filtro2([parametro_nivel]).then((data) => {
      ...
      console.log("Valor Filtro Ids: " + valor_filtro)
      console.log(identificadores);
      ...
      var largoTotal = 0;
      var xTotal = 0;
      console.log("RESULTADOS DE IDS FILTRO piso");
      console.log(resultado_ids);
...
async function buscarFiltros() {
  let data = await consulta_filtro2([]);
  let keys = Object.keys(data);
